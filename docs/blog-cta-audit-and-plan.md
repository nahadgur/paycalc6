...
**Status:** IMPLEMENTED 15 June 2026. Per-silo `cta` + 11 per-article overrides added to `lib/silos.ts` (`ctaForSpoke()`); both template CTAs wired in `app/blog/[slug]/page.tsx`; "Keep reading" now prefers same-silo posts; broken `/statutory-changes` link fixed to `/guides/statutory-changes`. The two body `href="/"` links in the NSSF post were left as-is: both are anchored "Kenya PAYE calculator" and `/` is the correct destination.
